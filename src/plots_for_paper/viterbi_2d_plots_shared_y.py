...
def plot_2d(xbefore,xafter,f,t,GW_f_true,vmin,vmax,GW_f_estim=None,fname=None,reverse_ordering=False):

    #Plot figure
    fig, (ax1,ax2) = plt.subplots(nrows=1, ncols=2, figsize=(17,9),sharey=True)


    pos1 = ax1.imshow(xbefore, cmap='viridis', interpolation='nearest',vmin=vmin,vmax=vmax,extent=[f[0],f[-1],t[0],t[-1]],aspect=1/200)
    pos2 = ax2.imshow(xafter, cmap='viridis', interpolation='nearest',vmin=vmin,vmax=vmax,extent=[f[0],f[-1],t[0],t[-1]],aspect=1/200)
    ...
    ax1.plot(GW_f_true,t,c='C2',linewidth=lw,linestyle=ls)
    ax2.plot(GW_f_true,t,c='C2',linewidth=lw,linestyle=ls)
    ax2.plot(GW_f_estim,t,c='y',linewidth=lw,linestyle=ls)
...
plot_2d(viterbi_with_pem.T,viterbi_without_pem.T,f,t,GW_freq_true,cbar_lower_limit,cbar_upper_limit,GW_freq_estim,fname="ANC_shared_y_example_1_low_contrast",reverse_ordering=True)
...
plot_2d(viterbi_with_pem.T,viterbi_without_pem.T,f,t,GW_freq_true,cbar_lower_limit,cbar_upper_limit,GW_freq_estim,fname="ANC_shared_y_example_1_high_contrast",reverse_ordering=True)
...
cbar_lower_limit = np.min(viterbi_with_pem)
cbar_upper_limit = np.max(viterbi_with_pem)
plot_2d(viterbi_with_pem.T,viterbi_without_pem.T,f,t,GW_freq_true,cbar_lower_limit,cbar_upper_limit,GW_freq_estim,fname="ANC_shared_y_example_2_low_contrast",reverse_ordering=True)
...
cbar_lower_limit = np.min(viterbi_without_pem)
cbar_upper_limit = np.max(viterbi_without_pem)
plot_2d(viterbi_with_pem.T,viterbi_without_pem.T,f,t,GW_freq_true,cbar_lower_limit,cbar_upper_limit,GW_freq_estim,fname="ANC_shared_y_example_2_high_contrast",reverse_ordering=True)
